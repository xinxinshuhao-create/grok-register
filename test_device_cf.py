"""
Device Code Flow + handle_cloudflare_challenge
"""
...
try:
    ...
    if "Attention Required" in page.title:
        print("遇到 CF 挑战，处理中...", flush=True)
        ok = page.handle_cloudflare_challenge(timeout=60)
        print(f"CF 处理结果: {ok}", flush=True)
        time.sleep(3)
        print(f"After CF: {page.url}", flush=True)
        print(f"Title: {page.title}", flush=True)

    # 如果到了设备 consent 页面，点 Allow
    if "device/consent" in page.url and "Attention Required" not in page.title:
        print("设备 consent 页面，点 Allow...", flush=True)
        try:
